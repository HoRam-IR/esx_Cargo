...
ESX.RegisterServerCallback("HR_Cargo:SyncBotsInServer", function(source, cb)
    KeyValue = {}
    for k, v in pairs(DoesBotHasKey) do
...
ESX.RegisterServerCallback("HR_Cargo:DoesBotHasKey", function(source, cb, ID)
    for k, v in pairs(DoesBotHasKey) do
...
ESX.RegisterServerCallback("HR_Cargo:GetBoxInventory", function(source, cb)
...
ESX.RegisterServerCallback("HR_Cargo:CheckCargoAvalibility", function(source, cb)
    Cops = 0
...
ESX.RegisterServerCallback("HR_Cargo:IsCodeValid", function(source, cb, Code)
...
ESX.RegisterServerCallback("HR_Cargo:MoneyRequest", function(source, cb)
...
RegisterServerEvent("HR_Cargo:TakeKeyCodeFromDealer")
AddEventHandler("HR_Cargo:TakeKeyCodeFromDealer", function(ID)
    local source = source
    for k, v in pairs(DoesBotHasKey) do
        if k == tonumber(ID) then
            v = false
...
RegisterServerEvent("HR_Cargo:ImArrived")
AddEventHandler("HR_Cargo:ImArrived", function()
    CargoTime = os.time()
    IsOnCoolDown = true
...
RegisterServerEvent("HR_Cargo:StartProcess")
AddEventHandler("HR_Cargo:StartProcess", function()
    for k, v in pairs(GetPlayers()) do
        local gPx = ESX.GetPlayerFromId(v)
        if gPx ~= nil then
            if gPx.job.name == 'police' then
    ...
    SetTimeout(900000, function()
        TriggerClientEvent("HR_Cargo:DropTheCargo", -1, Config.DropCoord)
    end)
end)

RegisterServerEvent('HR_Cargo:RefreshInventory')
AddEventHandler('HR_Cargo:RefreshInventory', function(type, name, number)
    print(true)
